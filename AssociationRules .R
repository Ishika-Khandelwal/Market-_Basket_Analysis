...
#  coerce the binary incidence matrix/array into a 'transactions' object. A transactions object
# is a special object specific to arules
...
benchmark_support <- itemFrequency(books.trans)

write(benchmark_support, file = "bechmark.csv", sep = ",")
View(benchmark_support)


# run apriori function
rules_2 <- apriori(books.trans, 
                 parameter = list(supp= 0.05, conf = 0.3, maxlen = 2, target = "rules"))

rules_3 <- apriori(books.trans, 
                 parameter = list(supp= 0.05, conf = 0.4, maxlen = 3, target = "rules"))
